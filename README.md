...
- **Chunked Upload Service** – `ChunkedUploadService` assembles large uploads, applies server-side checksum verification, and writes assembled ZIPs to S3-compatible storage. Upload job metadata lives in Redis so clients can resume progress instantly.
...
- **Tested Workflows** – Vitest covers controllers/services plus integration tests (`pnpm test:integration`) that exercise the real 107 MB ZIP pipeline, ensuring checksum, storage, and worker behaviors remain reliable.
