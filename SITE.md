...
- **Client (`apps/client/`)** – React 19 + Vite front end styled with Chakra UI. Key folders like `src/components/`, `src/pages/`, `src/hooks/`, and `src/lib/upload/uploadJobWorker.ts` handle gallery UI, auth context, upload progress workers, and route wiring. React Query + upload workers stream chunk progress back to the UI.
- **Server (`apps/server/`)** – Express 5 API plus background workers. `src/controllers/`, `src/handlers/`, `src/services/`, `src/middleware/`, and `src/workers/` cover HTTP flows, chunked uploads, Redis-backed queues, and gradient generation. Vitest (unit + `*.integration.test.ts`) and scripts under `apps/server/scripts/` support operational tasks.
- **Shared Utilities (`packages/utils/`)** – Workspace package with Zod schemas, DTOs, and type helpers consumed by both client and server to keep validation/payloads consistent.
...
- **Gradient Worker Pipeline** – `apps/server/src/workers/gradient.ts` dequeues image jobs from Redis, generates blur placeholders + gradients, and stores results via `GradientMetaService` for fast client rendering.
...
- **Shared Validation + Types** – Zod schemas in `packages/utils/` compile into TypeScript types that React forms, Express handlers, and workers all share, eliminating drift between front end and API contracts.
